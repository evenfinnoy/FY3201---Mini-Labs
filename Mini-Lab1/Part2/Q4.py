...
from external_data import load_cocoa_production_annual
...
def main():
    ...
    ax_ts2 = ax_ts.twinx()
    ax_ts.plot(harmonized.index, harmonized["Temperature"], color="tab:red", linewidth=2)
    ax_ts2.plot(harmonized.index, harmonized["Cocoa"], color="saddlebrown", linewidth=2)
    ...
    ax_scatter.scatter(harmonized["Cocoa"], harmonized["Temperature"], color="saddlebrown", s=16, alpha=0.7)
    ...
    ax_scatter.plot(x_fit, y_fit, color="black", linewidth=1.5)
    ...
    fig.text(
        ...
        "consumption, since a long clean consumption series is not readily available).",
